...
export const searchQuerySchema = baseSearchSchema.transform((val: string) => val.trim());

// Schéma spécifique pour la recherche de produits/deals
export const productSearchSchema = baseSearchSchema
  .refine((val: string) => !/<[^>]*>/.test(val), 'HTML non autorisé dans la recherche')
  .transform((val: string) => val.trim());

// Schéma pour la recherche d'articles de blog  
export const blogSearchSchema = baseSearchSchema
  .refine((val: string) => !/(javascript:|data:|vbscript:)/i.test(val), 'Contenu dangereux détecté')
  .transform((val: string) => val.trim());

// Schéma pour la recherche dans les favoris
export const favoritesSearchSchema = baseSearchSchema.transform((val: string) => val.trim());
...
export function validateSearch(query: string, schema = searchQuerySchema): string {
  try {
...
export function isValidSearch(query: string, schema = searchQuerySchema): boolean {
  try {
    schema.parse(query);
    return true;
  } catch {
